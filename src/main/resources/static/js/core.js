...
var Core = (function () {
    ...
    /*ajax请求*/
    core.postAjax = function (url, dataToPost, fn, type, contentType, async) {
        $.ajax({
            url: url,
            cache: false,
            ...
            error:function (XMLHttpRequest, textStatus, errorThrown) {
                if(XMLHttpRequest.status==403){
                    ...
                    layer.msg("Server internal error！")
                }else{
                    layer.msg("Server unknown error！")
                }
            }
        });
    };
    ...
    core.initTable = function (options, success) {
        ...
        $(tableOptions.id).bootstrapTable({
            ...
            fixedColumns: tableOptions.fixedColumns,
            fixedNumber: tableOptions.fixedNumber,
            fixedRightNumber: tableOptions.fixedRightNumber
            ,locale:"en-US"
        });
    }
    ...
    core.selectSingleData = function (id){
        ...
        if(typeof(selectContent) == 'undefined' || selectContent == "") {
            layer.msg("Please select a piece of data!");
            return false;
    ...
    core.buildTree= function (array,id,parent_id,childName) {
        // 创建临时对象
        let temp = {};
        // 创建需要返回的树形对象
        let tree = {};
        ...
        for(let i in temp) {
            ...
            if(temp[i][parent_id] !== null) {
                if(temp[temp[i][parent_id]] ){
                    if( !temp[temp[i][parent_id]][childName]) {
                        temp[temp[i][parent_id]][childName] = new Array();
                    }
                    temp[temp[i][parent_id]][childName].push(temp[i]);
                }
            } else {
